"""Factories that turn :mod:`astraeval.config.schema` dataclasses into runtime objects."""
...
from astraeval.metrics.base import Metric
...
def build_provider(cfg: ProviderConfig) -> Provider:
    ...
    :rtype: Provider
    :raises astraeval.exceptions.ConfigError: When a ``fake`` provider is
...
def build_dataset(cfg: DatasetConfig) -> Iterable[Sample]:
    ...
    :raises astraeval.exceptions.ConfigError: When the dataset type is
        unknown.
    """
    if cfg.type == "jsonl":
...
def build_metric(cfg: MetricConfig, *, judge: LLMJudge | None = None) -> Metric:
    ...
    :rtype: Metric
    :raises astraeval.exceptions.ConfigError: When an LLM-as-judge metric
...
def build_eval_run(cfg: RunConfig) -> EvalRun:
    ...
    :rtype: EvalRun
    :raises astraeval.exceptions.ConfigError: When metric configuration
