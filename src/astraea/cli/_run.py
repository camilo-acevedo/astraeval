"""Implementation of the ``astraea run`` subcommand."""
...
from astraea.config.builder import build_eval_run
from astraea.config.loader import load_yaml
from astraea.config.schema import RunConfig
from astraea.core.eval_run import RunResult
from astraea.exceptions import ThresholdError
from astraea.reports.html_report import write_html
from astraea.reports.json_report import write_run


def run_command(args: argparse.Namespace) -> int:
    """Execute the ``astraea run`` workflow and return an exit code.
    ...
    :rtype: int
    :raises astraea.exceptions.ThresholdError: When at least one
        configured threshold was not met.
    :raises astraea.exceptions.AstraeaError: For any other domain error
