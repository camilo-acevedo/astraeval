"""Command-line interface for the ``astraea`` package."""
...
from astraea.cli._diff import diff_command
from astraea.cli._run import run_command
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="astraea",
        description="Astraea: reproducible LLM evaluation harness.",
    )
    parser.add_argument(
        "-V",
        "--version",
        action="version",
        version=f"astraea {__version__}",
    )
    ...
    diff.add_argument("run_a", help="Baseline run directory (output of astraea run).")
...
def main(argv: list[str] | None = None) -> int:
    """Entry point used by both the console script and ``python -m astraea``.
