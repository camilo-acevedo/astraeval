"""Implementation of the ``astraea diff`` subcommand."""
...
from astraea.exceptions import AstraeaError, ThresholdError
...
def diff_command(args: argparse.Namespace) -> int:
    ...
    :param args: Parsed command-line arguments produced by
        :func:`astraea.cli.build_parser`.
    :type args: argparse.Namespace
    :returns: Process exit code.
    :rtype: int
    :raises astraea.exceptions.ThresholdError: When any metric regressed
        by more than ``args.max_regression``.
    :raises astraea.exceptions.AstraeaError: When either run directory
        cannot be loaded.
    """
...
def _load_summary(run_dir: str) -> dict[str, Any]:
    ...
    :param run_dir: Filesystem path to a per-run output directory produced
        by :func:`astraea.reports.json_report.write_run`.
    :type run_dir: str
    :returns: Parsed summary JSON document.
    :rtype: dict[str, Any]
    :raises astraea.exceptions.AstraeaError: When the directory or
        summary file cannot be located, or when the JSON is malformed.
    """
    path = Path(run_dir)
    if not path.is_dir():
        raise AstraeaError(f"Run directory not found: {run_dir}")
    summary_path = path / "summary.json"
    if not summary_path.is_file():
        raise AstraeaError(f"summary.json missing in {run_dir}")
    try:
        data = json.loads(summary_path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise AstraeaError(f"Invalid JSON in {summary_path}: {exc.msg}") from exc
    if not isinstance(data, dict):
        raise AstraeaError(f"summary.json in {run_dir} must be a JSON object.")
    return data
